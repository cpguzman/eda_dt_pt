...
data['cps_fases'] = pd.read_csv('cps_fases.csv')
...
model.faseconnected = pyo.Param(model.cp, initialize =_auxDictionary(data['cp_inputs'].to_numpy()[:,8]))
...
model.cpfases = pyo.Param(model.cp, model.f, initialize = _auxDictionary(data['cps_fases'].to_numpy()))
...
model.Pcsmax = pyo.Param(model.cs, initialize =_auxDictionary(data['css_inputs'].to_numpy()[:,1])) #Caso da estação ser equilibrada nas três fases
#model.Pcsmax = pyo.Param(model.cs, model.f, initialize = _auxDictionary(data['css_power'].to_numpy())) #Caso da estação poder ser dequilibrada nas três fases
...
def _cp_power_charging_limit(m,ev,cp,f,t): 
    if m.type_[cp] == 1 and m.place[cp] == m.ev_id[ev]:  
        ...
        return m.PEV[ev,t] == m.Pcpmax[cp]*m.alpha[ev,t]*m.a[ev,t]
    return pyo.Constraint.Skip
model.cp_power_charge_limit = pyo.Constraint(model.ev, model.cp, model.f, model.t, rule = _cp_power_charging_limit)
...
def _cp_power_discharging_limit(m,ev,cp,f,t): 
    #print({m.type_})
    if m.type_[cp] == 1 and m.place[cp] == m.ev_id[ev]: 
        ...
        #return 0 
    return pyo.Constraint.Skip      
model.cp_power_discharge_limit = pyo.Constraint(model.ev, model.cp, model.f, model.t, rule = _cp_power_discharging_limit)
...
def _cs_power_charging_limit(m,t,cp, cs): 
    if m.cs_id[cs] == m.my_cs_id_cp[cp]: 
        ...
        return sum([m.PCP[cp,t] - m.PCPdc[cp,t] for cp in m.cp if m.cs_id[cs] == m.my_cs_id_cp[cp]]) <= m.Pcsmax[cs]/3
    else:
        return m.PCP[cp,t] - m.PCPdc[cp,t] <= m.Pcsmax[cs] 
    #return pyo.Constraint.Skip    
...
#def _cs_power_charging_limit(m,f,t,cp, cs): 
#    if m.cs_id[cs] == m.my_cs_id_cp[cp]: 
#        return m.PCP[cp,t] - m.PCPdc[cp,t] <= m.Pcsmax[cs,f] 
#    return pyo.Constraint.Skip    
#model.cs_power_charging_limit = pyo.Constraint(model.f, model.t, model.cp, model.cs, rule = _cs_power_charging_limit)  
...
def _cs_power_discharging_limit(m,t,cs): 
    if m.cs_id[cs] == m.my_cs_id_cp[cp]: 
        soma = 0
        for ev in m.ev:
            if m.cpconnected[ev] == m.cp_id[cp]: 
        ...
        return m.PCP[cp,t] - m.PCPdc[cp,t] >= -1*m.Pcsmax[cs]/3 
    return pyo.Constraint.Skip    
#model.cs_power_discharging_limit = pyo.Constraint(model.f, model.t, model.cs, rule = _cs_power_discharging_limit) 
model.cs_power_discharging_limit = pyo.Constraint(model.t, model.cs, rule = _cs_power_discharging_limit)  


#def _cs_power_discharging_limit(m,f,t,cs): 
#    if m.cs_id[cs] == m.my_cs_id_cp[cp] and model.faseconnected[cp] == model.csconnected[cp]: 
#        return m.PCP[cp,t] - m.PCPdc[cp,t]  >= -1*m.Pcsmax[cs,f]  
#    return pyo.Constraint.Skip    
#model.cs_power_discharging_limit = pyo.Constraint(model.f, model.t, model.cs, rule = _cs_power_discharging_limit)  
